...
# --- 1. VANILLA AUTOENCODER MİMARİSİ (Eğitim koduyla birebir aynı) ---
...
class VanillaAutoencoder(nn.Module):
    def __init__(self, in_channels=1, out_channels=3):
        ...
        # Decoder (Doğrusal Akış - Skip Connection Yok)
...
def load_model(weights_path, device):
    ...
    # Eğitim kodunda 'model_state_dict' olarak kaydedildiği için direkt yüklüyoruz.
...
def show_results_inline(nir, pred, gt, title_info, img_name):
    ...
    def on_key(event):
        if event.key == 'k':
            save_dir = "kaydedilen_gorseller_vanilla_Autoencoder"
            ...
            nir_path = os.path.join(save_dir, f"{dosya_adi_uzantisiz}_VANILLA_AUTOENCODER_NIR_INPUT.tif")
            ...
            pred_path = os.path.join(save_dir, f"{dosya_adi_uzantisiz}_VANILLA_AUTOENCODER_PREDICTED_RGB.tif")
            ...
            gt_path = os.path.join(save_dir, f"{dosya_adi_uzantisiz}_VANILLA_AUTOENCODER_GROUND_TRUTH.tif")
...
if __name__ == "__main__":
    ...
    # Ağırlık ismi eğitim çıktısına ("VanillaAutoEncoder_epoch_X.pth") uyarlandı.
    WEIGHTS_PATH = "VanillaAutoEncoder_epoch_3500.pth"

    DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    print(f"Cihaz: {DEVICE} üzerinde Vanilla Autoencoder modeli yükleniyor...")

...
# --- 1. S-NET MİMARİSİ (TEST İÇİN SADECE GENERATOR) ---
...
def load_model(weights_path, device):
    ...
    #  SNetGenerator çağrılıyor
    model = SNetGenerator(1, 3).to(device)
...
def process_and_evaluate(model, device, nir_path, rgb_path):
    ...
    # 3. Tensörü Görüntü formatına çevir
...
def show_results_inline(nir, pred, gt, title_info, img_name):
    ...
    def on_key(event):
        if event.key == 'k':
            save_dir = "kaydedilen_gorseller_SNET"
            ...
            nir_path = os.path.join(save_dir, f"{dosya_adi_uzantisiz}_S-NET_NIR_OUTPUT.tif")
...
if __name__ == "__main__":
    # Test klasörü ve ağırlık dosyasının yolunu kendine göre güncelle
    ...
    # Ağırlık ismini kendi kaydettiğin formata göre değiştir (Örn: S-Net_epoch_3500.pth)
    WEIGHTS_PATH = "S-Net_epoch_3500.pth"

    DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    print(f"Cihaz: {DEVICE} üzerinde S-Net modeli yükleniyor...")

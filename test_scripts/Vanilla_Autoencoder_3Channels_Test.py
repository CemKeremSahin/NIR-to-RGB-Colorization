...
# --- 1. VANILLA AUTOENCODER MİMARİSİ (Eğitim kodundan alındı) ---
...
def load_model(weights_path, device):
    ...
    # Mimari VanillaAutoencoder olarak değiştirildi
    model = VanillaAutoencoder(in_channels=3, out_channels=3).to(device)
...
def show_results_inline(nir_vis, pred, gt, title_info, img_name):
    ...
    plt.subplot(1, 3, 2)
    plt.imshow(pred)
    plt.title(f'Predicted RGB (Vanilla AE)\n{title_info}', fontsize=12, color='blue')
    plt.axis('off')
    ...
    def on_key(event):
        if event.key == 'k':
            save_dir = "kaydedilen_gorseller_VANILLA_AE_3CH"
            os.makedirs(save_dir, exist_ok=True)

            dosya_adi_uzantisiz, _ = os.path.splitext(img_name)

            nir_bgr = cv2.cvtColor(nir_vis, cv2.COLOR_RGB2BGR)
            nir_path = os.path.join(save_dir, f"{dosya_adi_uzantisiz}_VANILLA_AE_NIR_COMPOSITE.tif")
            cv2.imwrite(nir_path, nir_bgr)

            pred_bgr = cv2.cvtColor(pred, cv2.COLOR_RGB2BGR)
            pred_path = os.path.join(save_dir, f"{dosya_adi_uzantisiz}_VANILLA_AE_PREDICTED_RGB.tif")
            cv2.imwrite(pred_path, pred_bgr)

            gt_bgr = cv2.cvtColor(gt, cv2.COLOR_RGB2BGR)
            gt_path = os.path.join(save_dir, f"{dosya_adi_uzantisiz}_VANILLA_AE_GROUND_TRUTH.tif")
...
if __name__ == "__main__":
    ...
    # Ağırlık dosyasının ismi, klasöre attığın pth dosyasının adıyla aynı olmalı
    WEIGHTS_PATH = "VanillaAutoEncoder_3Channels_epoch_3500.pth"

    DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    print(f"Cihaz: {DEVICE} üzerinde 3 Kanallı Vanilla Autoencoder modeli yükleniyor...")
    model_G = load_model(WEIGHTS_PATH, DEVICE)
    print("Vanilla Autoencoder Modeli başarıyla yüklendi!\n")

...
# --- 1. U-NET MİMARİSİ (TEST İÇİN SADECE GENERATOR) ---
# Eğitim kodundaki yapının birebir aynısı eklendi.
...
class UNetGenerator(nn.Module):
    ...
    def forward(self, x):
        ...
        # Decoder aşaması (Skip connection'lar ile birleştirme)
        x = self.up1(x5)
...
def load_model(weights_path, device):
    ...
    # SNet yerine UNetGenerator çağrılıyor
...
def process_and_evaluate(model, device, nir_path, rgb_path):
    ...
    # Tıpkı eğitimdeki gibi veriyi [0, 1] aralığına ve ardından [-1, 1] aralığına çekiyoruz.
    ...
    # 3. Tensörü Görüntü formatına çevir ( [-1, 1] aralığını tekrar [0, 1] aralığına alıp 255 ile çarpıyoruz )
...
def show_results_inline(nir, pred, gt, title_info, img_name):
    ...
    def on_key(event):
        if event.key == 'k':
            save_dir = "kaydedilen_gorseller_UNET-GAN"
            ...
            nir_path = os.path.join(save_dir, f"{dosya_adi_uzantisiz}_UNET_NIR_INPUT.tif")
...
if __name__ == "__main__":
    ...
    # Ağırlık ismini eğitim çıktısına uyarladık. (İstediğin epoch numarasını buraya yazabilirsin)
    WEIGHTS_PATH = "UNetGAN_Realword_epoch_3500.pth"
